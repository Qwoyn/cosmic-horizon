...
type MallTab = 'trade' | 'store' | 'dealer' | 'garage' | 'salvage' | 'cantina' | 'refuel';
...
export default function MallPanel({ outpostId, onBuy, onSell, credits, energy, maxEnergy, playerLevel, onAction, bare }: Props) {
      ...
      {TABS.map((t, i) => (
        <span key={t.key}>
          ...
          <span
            ...
            style={{ cursor: 'pointer', color: tab === t.key ? '#0f0' : '#666' }}
          >
  ...
  const renderTab = () => {
    switch (tab) {
      case 'trade':
        return <TradeTable outpostId={outpostId} onBuy={onBuy} onSell={onSell} bare />;
      case 'store':
        return <MallStoreTab credits={credits} onAction={onAction} />;
      case 'dealer':
        return <MallDealerTab credits={credits} playerLevel={playerLevel} onAction={onAction} />;
      case 'garage':
      ...
      case 'refuel':
        return <MallRefuelTab energy={energy} maxEnergy={maxEnergy} credits={credits} onAction={onAction} />;
    }
  };
