...
const SEPARATOR_AFTER = new Set([1, 5, 9, 11]);

export default function ActivityBar({ activePanel, onSelect, badges }: ActivityBarProps) {
  const [hoveredIndex, setHoveredIndex] = useState<number | null>(null);
  const [tooltipPos, setTooltipPos] = useState<{ top: number; left: number }>({ top: 0, left: 0 });

  const handleMouseEnter = useCallback((e: React.MouseEvent<HTMLButtonElement>, idx: number) => {
        ...
        {PANELS.map((p, i) => {
          ...
          return (
            <React.Fragment key={p.id}>
              ...
              {SEPARATOR_AFTER.has(i) && <div className={`activity-bar__sep${i === 1 ? ' activity-bar__sep--primary' : ''}`} />}
            </React.Fragment>
          );
        })}
      </div>
