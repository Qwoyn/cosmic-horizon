...
export default function MallDealerTab({ credits, playerLevel = 1, onAction }: Props) {
      ...
      {ships.length === 0 ? (
        ...
        <div className="mall-item-list">
          {ships.map(ship => {
            ...
            return (
              <div key={ship.id} className={`mall-item${locked ? ' mall-item--locked' : ''}`}>
                <div className="mall-item__info">
                  <span className="mall-item__name">{ship.name}</span>
                  <span className="mall-item__stats">
                    W:{ship.weaponSlots} C:{ship.cargoCapacity} E:{ship.enginePower} HP:{ship.hullHp}
                  </span>
                  {locked && <span className="mall-item__locked">Requires Level {ship.levelRequired}</span>}
                </div>
                <div className="mall-item__action">
                  <span className="mall-item__price">{ship.price.toLocaleString()} cr</span>
                  <button
                    className="btn-sm btn-buy"
                    disabled={locked || buying === ship.id || credits < ship.price}
