...
type TabView = 'overview' | 'governance' | 'economy' | 'projects' | 'structures' | 'admin';
...
export default function SyndicateGroupPanel({ refreshKey, onCommand, bare }: Props) {
  ...
  const myMembership = members.find((m: any) => m.id === syndicateData?.leaderId);
  const isLeader = syndicateData?.leaderId && members.some((m: any) => m.id === syndicateData.leaderId && m.role === 'leader');
  ...
  const tabs = showAdmin ? [...baseTabs, { key: 'admin' as TabView, label: 'Admin' }] : baseTabs;

  // If not in a syndicate, only show overview
  const activeTabs = syndicateData ? tabs : [{ key: 'overview' as TabView, label: 'Overview' }];
  ...
  const content = (
    <>
      {tabBar}
      {tab === 'overview' && <SyndicateOverviewPanel refreshKey={effectiveKey} onCommand={onCommand} bare onRefresh={triggerRefresh} />}
      {tab === 'governance' && syndicateId && <GovernanceTab syndicateId={syndicateId} refreshKey={effectiveKey} bare />}
      {tab === 'economy' && <SyndicateEconomyPanel refreshKey={effectiveKey} bare />}
      {tab === 'projects' && <SyndicateProjectsPanel refreshKey={effectiveKey} onCommand={onCommand} bare />}
      {tab === 'structures' && <SyndicateStructuresPanel refreshKey={effectiveKey} bare />}
