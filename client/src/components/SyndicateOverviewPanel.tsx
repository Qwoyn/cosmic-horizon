...
import {
  getSyndicate, getAlliances, createSyndicate, browseSyndicates,
  joinSyndicate, joinSyndicateByCode, depositToTreasury, withdrawFromTreasury,
  leaveSyndicate,
} from '../services/api';
...
export default function SyndicateOverviewPanel({ refreshKey, onCommand, bare, onRefresh }: Props) {
  ...
  useEffect(() => { fetchData(true); }, [refreshKey]);
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      knownInSyndicate.current = true;
      setSyndicate({ id: data.syndicateId, name: data.name, treasury: 0, members: [], settings: null });
      setMembers([]);
      onRefresh?.();
    } catch (err: any) {
  ...
  const handleJoin = async (syndicateId: string) => {
    ...
    try {
      ...
      setActionMsg(data.action === 'joined' ? 'Joined syndicate!' : 'Join request sent!');
      if (data.action === 'joined') {
        knownInSyndicate.current = true;
        setSyndicate({ id: data.syndicateId || syndicateId, name: '', treasury: 0, members: [], settings: null });
        onRefresh?.();
      }
    } catch (err: any) {
  ...
  const handleJoinByCode = async () => {
    ...
    try {
      ...
      knownInSyndicate.current = true;
      setSyndicate({ id: data.syndicateId || '', name: '', treasury: 0, members: [], settings: null });
      onRefresh?.();
    } catch (err: any) {
  ...
  if (!syndicate) {
    const content = (
      <>
        <div className="text-muted" style={{ marginBottom: 8, fontStyle: 'italic' }}>
          Form or join a syndicate to pool resources, govern territories, and dominate the galaxy.
        </div>

        {error && <div style={{ color: 'var(--red)', marginBottom: 4 }}>{error}</div>}
        ...
        <div className="syndicate-create-form">
          <input placeholder="Syndicate name" value={createName} onChange={e => setCreateName(e.target.value)} className="chat-input" style={{ marginBottom: 4 }} />
          <input placeholder="Motto (optional)" value={createMotto} onChange={e => setCreateMotto(e.target.value)} className="chat-input" style={{ marginBottom: 4 }} />
          <div className="panel-row" style={{ gap: 4, flexWrap: 'wrap' }}>
            <label style={{ fontSize: 10, color: '#888' }}>Recruitment:</label>
            <select value={createMode} onChange={e => setCreateMode(e.target.value)} className="qty-input" style={{ width: 'auto' }}>
              <option value="open">Open</option>
              <option value="closed">Closed</option>
              <option value="invite_only">Invite Only</option>
            </select>
            <label style={{ fontSize: 10, color: '#888' }}>Min Lvl:</label>
            <input type="number" min={1} max={100} value={createMinLevel} onChange={e => setCreateMinLevel(Number(e.target.value) || 1)} className="qty-input" style={{ width: 40 }} />
          </div>
          <div className="panel-row" style={{ gap: 4, flexWrap: 'wrap', marginTop: 4 }}>
            <label style={{ fontSize: 10, color: '#888' }}>Quorum %:</label>
            <input type="number" min={1} max={100} value={createQuorum} onChange={e => setCreateQuorum(Number(e.target.value) || 60)} className="qty-input" style={{ width: 40 }} />
            <label style={{ fontSize: 10, color: '#888' }}>Vote hrs:</label>
            <input type="number" min={1} max={720} value={createVoteDuration} onChange={e => setCreateVoteDuration(Number(e.target.value) || 48)} className="qty-input" style={{ width: 50 }} />
          </div>
          <button className="btn-sm btn-buy" onClick={handleCreate} disabled={creating} style={{ marginTop: 6 }}>
            {creating ? 'CREATING...' : 'CREATE'}
          </button>
        </div>

        {/* Browse Syndicates */}
        <div className="panel-subheader" style={{ marginTop: 8 }}>Browse Syndicates</div>
        <div className="panel-row" style={{ gap: 4 }}>
          <input placeholder="Search..." value={browseSearch} onChange={e => setBrowseSearch(e.target.value)} className="chat-input" style={{ flex: 1 }} />
          <select value={browseMode} onChange={e => setBrowseMode(e.target.value)} className="qty-input" style={{ width: 'auto' }}>
            <option value="all">All</option>
            <option value="open">Open</option>
            <option value="closed">Closed</option>
          </select>
          <button className="btn-sm btn-buy" onClick={handleBrowse} disabled={browsing}>
            {browsing ? '...' : 'SEARCH'}
          </button>
        </div>
            ...
            {browseResults.map(s => (
              <div key={s.id} className="syndicate-browse-item">
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ color: 'var(--magenta)', fontWeight: 'bold' }}>{s.name}</span>
                  <span style={{ fontSize: 9, color: '#888' }}>
                    {s.member_count} members
                    {s.min_level > 1 && ` · Lvl ${s.min_level}+`}
                  </span>
                </div>
                {s.motto && <div style={{ fontSize: 10, color: '#aaa', fontStyle: 'italic' }}>{s.motto}</div>}
                <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 2 }}>
                  <span className={`badge-recruitment badge-${s.recruitment_mode || 'closed'}`}>
                    {(s.recruitment_mode || 'closed').toUpperCase()}
                  </span>
                  <button className="btn-sm btn-buy" onClick={() => handleJoin(s.id)}>
                    {(s.recruitment_mode || 'closed') === 'open' ? 'JOIN' : 'REQUEST'}
                  </button>
                </div>
              </div>
            ))}
        ...
        {/* Join by Code */}
        <div className="panel-subheader" style={{ marginTop: 8 }}>Join by Invite Code</div>
        <div className="panel-row" style={{ gap: 4 }}>
          <input placeholder="Enter code..." value={inviteCode} onChange={e => setInviteCode(e.target.value)} className="chat-input" style={{ flex: 1 }} />
          <button className="btn-sm btn-buy" onClick={handleJoinByCode}>JOIN</button>
        </div>
      </>
    );
    ...
    <>
      ...
      {syndicate.settings?.motto && (
        <div className="panel-row">
          ...
          <span style={{ color: '#aaa', fontStyle: 'italic' }}>{syndicate.settings.motto}</span>
        </div>
      )}
      <div className="panel-row">
      ...
      {syndicate.settings && (
        <div className="panel-row">
          ...
          <span className={`badge-recruitment badge-${syndicate.settings.recruitment_mode}`}>
      ...
      <div className="panel-row" style={{ gap: 4 }}>
        <input type="number" min={1} value={depositAmt} onChange={e => setDepositAmt(Math.max(1, parseInt(e.target.value) || 1))} className="qty-input" style={{ width: 70 }} />
        <button className="btn-sm btn-buy" onClick={handleDeposit}>DEPOSIT</button>
        <input type="number" min={1} value={withdrawAmt} onChange={e => setWithdrawAmt(Math.max(1, parseInt(e.target.value) || 1))} className="qty-input" style={{ width: 70 }} />
      ...
      <div style={{ marginTop: 12 }}>
        <button className="btn-sm" style={{ background: '#400', color: 'var(--red)' }} onClick={handleLeave}>
          LEAVE SYNDICATE
        </button>
      </div>
    </>
