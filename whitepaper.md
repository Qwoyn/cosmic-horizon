...
**A strategy and trade-based cosmic multi-player game, built on the QWOYN Network**

*By: W. Moglia, D. Pittman, S. Lott*
...
The two mechanisms, or economics catalysts are (a) in-game economy (IGE) bootstrapping pool and (b) periodic outpost treasury injections via the game tick system. More on these mechanisms can be found in [Section 4](#4-cosmic-horizon-economics).
...
**Example A:** The player is in sector 220, they have the option to travel to any of the adjacent sectors (in this case, sectors 86 and 316). Assuming the player travels from sector 220 to sector 86, they will then have the option to navigate to sectors 100, 316 or 220.

##### 3.2.1.2 Special Sectors (One-Way / "Drop Offs")
...
If the player finds themselves in sector 300, they can move to sector 350, but cannot travel back to sector 300 directly. In order to return to sector 300 they would need to travel from 350 -> 402 -> 450 -> 300.
...
- Sectors containing a Star Mall

##### 3.2.1.5 Harmony Enforced Routes
...
One of the first ways a player might earn funds is by locating outposts and engaging in [celestial arbitrage](#13-celestial-arbitrage).

##### 3.3.1.1 Refueling Stations
...
If a planet is uninhabited, it can simply be claimed by any player that lands and lays claim to it. If a planet is inhabited, but undefended, players may need to win the loyalty of the inhabitants with either a monetary or military inducement. Militarized planets must be subdued by force before they can be claimed.

##### 3.3.3.1 Planet Types

There are eight planetary types to discover or create in the galaxy:

| Planet Class | Planet Type |
|:---:|:---|
| Class H | Goldilocks planet (GLP / hospitable) |
| Class D | Desert Planet |
| Class O | Ocean Planet |
| Class A | Alpine Planet |
| Class F | Frozen Planet |
| Class V | Volcanic Planet |
| Class G | Gaseous Planet |
| Class S | Colonist Seed Planet |

Each planet, when colonized, will produce different commodities and/or drones at different rates. For example, if a Class H and Class D planet were equally colonized, the Class H would produce a higher daily yield in food commodities whereas the desert planet would produce a higher daily yield in cyrillium.
...
|:---:|:---|:---|
| 0 | Undeveloped | Most planets discovered will be level zero, either uninhabited or containing a small population with no industrial development. Such planets can be easily claimed with little to no resistance from the population. |
| 1 | Metropolis | After a planet reaches a sufficient population it can be upgraded to Level 1, whereby a Metropolis is constructed. A metropolis in itself is little more than a city with infrastructure for further upgrade. This is a sensitive stage for a developing planet. |
| 2 | Militarization | Allows for fortifications to be deployed and adds a combat tractor beam to the planet, allowing any tactical drones in the planet's inventory to be activated. Using the militarization menu, the player can set drones to different defensive postures and reaction levels. |
| 3 | Atmospheric Defenses | Large plasma cannons which fire on unfriendly ships when they attempt to pass through the planet's atmosphere (i.e. land on the planet). They have their own power banks which must be charged. This upgrade also enables players to turn planetary cyrillium into refined, weapons-grade cyrillium. |
| 4 | Planetary-based Sector Defenses (PSDs) | An upgrade to Atmospheric Defenses. PSDs extend the range of the plasma cannons so they can fire at unfriendly ships passing through the same sector. PSDs can be set to different intensity levels. They deliver less damage per unit of energy expended compared to Atmospheric Defenses. |
| 5 | Planetary Shield System | A protective energy shield around the planet which must be defeated by invading players before they are able to land. Shield systems draw from the planetary reserve of refined cyrillium. |
| 6 | Advanced Atmospheric Tractor Beam (AATB) | When engaged the AATB will grab and hold ships that pass through the sector where the planet is located. |
| 7 | Planetary Warp | Allows an entire planet to be moved from one sector to another. Great care should be taken -- warping into an uncharted sector can result in the planet colliding with another object yielding very destructive results. |
...
The terraforming process involves purchasing real-world carbon credits and retiring them. For more on terraforming, see [Section 5, Ecology](#5-ecology).

#### 3.3.5 Star Malls

Star Malls are an important strategic and social hub in the Cosmic Horizon universe. The layout of a Star Mall may vary, but each may feature one or more of the following areas:

##### 3.3.5.1 Ship Dealer

Where players can come to purchase new ships. Different ship dealers may have different ships on offer. If a player has captured a ship, this is also where a player can officially claim it, by registering it in their own name.

##### 3.3.5.2 Ship Repair and Salvage Yard

Where a player can sell captured or damaged ships, as well as sell discovered artifacts or derelict ships left behind by an as yet unidentified alien race.

##### 3.3.5.3 Star Mall Garage
...
##### 3.3.5.4 Refueling Stations

Refueling stations exist throughout the galaxy, in the form of special outposts. Every Star Mall is also equipped with these facilities.

##### 3.3.5.5 General Store

The general store sells a number of items and accessories to enhance gameplay and ship performance:

- **Planet Generating Devices (PGD):** Fired by ships capable of carrying them to generate new planets. Life from lifelessness.
- **Space Buoys:** Simple cosmic sign posts. Players can leave a custom message on the buoys for passing ships to inspect. They can also be utilized to triangulate a cross-cosmos jump, for ships equipped with Jump Drives.
...
- **Cloaking Devices:** (available on certain ships) Allow the player to cloak their ship, rendering it mostly invisible. This makes parking a ship in open space significantly safer. It is not infallible however. Cloaked ships might be detected by some scanners, if the user has a careful eye and knows what to look for. Ships cannot move or engage in combat while cloaked.
- **Cloaking Power Cells:** Each use of the cloaking device burns a power cell.
- **Exploratory Probes:** Probes that a player can send off toward a distant sector, which send back a report of the sectors they pass through. Probes have no defenses and self-destruct if they encounter obstacles. That said, with a little luck, a probe is occasionally able to slip through a fortified sector.
- **Planetary Scanner:** (limited to certain ships) Allow the user to get an understanding of the planets in a sector without landing on them, such as planetary development and military readiness.
...
- **Sector Scanner:** Give the player an idea of what lies in their adjacent sectors. The scanners are rudimentary however, and require some care to interpret their readouts correctly. For experienced players, they are possibly more comprehensive than advanced sector scanners.
- **Advanced Sector Scanner:** Similarly give the player a view on what lies in adjacent sectors, however the readout is much more user friendly. Advanced sector scanners are more expensive than standard scanners.
...
- **Drone Pack:** Drones are available with three different power levels and can be deployed in offensive, defensive or toll mode. Most ships can only carry one drone at a time.

##### 3.3.5.6 Star Mall Cantina

Every Star Mall needs a wretched hive for the inhabitants of the galaxy to whet their whistle. The cantina is the spot for that and more. What secrets might be hidden in this dark and seedy corner of the galaxy?

##### 3.3.5.7 Justice Center
...
In the Justice Center players can place a **bounty** on another player or a reward for the destruction of a specific ship. If any player successfully kills a wanted player or destroys a ship with a bounty, they can visit the justice center to collect the reward.
...
The in-game currency is $COHO. $COHO is used to purchase commodities, ships, ship upgrades, special items and to complete most other transactions in the game. Commodities sold to outposts are paid for in $COHO.
...
Each ship has some universal properties: each has an engine system with its own power banks, a combat tractor beam and a weapons system which shares its power banks with a nominal shielding system. With the exception of DodgePods, every ship is capable of carrying goods in its storage holds.
...
- **Navigation:** A player can quickly navigate to adjacent sectors with a simple click. Alternatively, they can plot a course to a distant sector and the ship's navigation computer will calculate a path to the sector in question. Different route options will be displayed if available.
- **Dock / Orbit:** If an outpost is in the sector, the player has the option to dock or orbit and engage in commerce. If there is a Star Mall in the sector, the player can land on the Star Mall and explore what it has to offer.
- **Landing Sequence:** If one or more planets are in a sector, the player can orbit or land on the planet to access planetary controls.
- **Scan:** With a scanner, which will need to be acquired in game, a player can get an idea of what hazards may exist in adjacent sectors. Different scanners will have a greater range and offer more or less user-friendly visualizations of the contents of scanned sectors.
- **Deploy Drones:** If a player has acquired drones, they may deploy them in the current sector and set them to the desired mode:
  - **Offensive drones** will immediately attack any non-aligned player that enters the sector. If the drones destroy a ship, the ejected DodgePod will always successfully escape. If a DodgePod enters another sector with offensive drones however, it may be destroyed.
  - **Defensive drones** do not immediately attack players that enter the sector, but rather block the player from passing through to other adjacent sectors. The player can retreat from the sector they came from or attack the defensive drones.
  - **Toll drones** act similarly to defensive drones, but the player that encounters them has the option to pay a toll. If the toll is paid, the player is allowed to pass through the sector for a set amount of time.
- **Buoy Deployment:** If a player has purchased space buoys, they can deploy them. Buoys are simple space sign posts with a custom message. Additionally, buoys keep a log ("buoy report") of the ships that pass by which the player can review.
- **Tow Beam:** Ships with towing beams are capable of towing other ships. Towing other ships will increase fuel consumption, depending on the size of the ship being towed.
- **Teleporter:** A player can use their ship's transporter to send their avatar from one ship to another, provided the other ship belongs to them or is disabled.
- **Eject Cargo:** If for some reason the player's cargo holds are fully occupied and there is no opportunity to sell or unload them, the player may elect to eject their cargo into space.
- **Combat:** Engage another ship in combat. For details, see [Section 3.6.4](#364-combat).
...
Ships come with set weapon and storage minimum values that can be upgraded within the limits of their ship's specifications. Different ships will come with different initial settings and different upgrade limits.

**Generic Example Ship:**

| Stat | Initial Max | Upgraded Max |
|:---|:---:|:---:|
| Weapons System Energy | 75 | 250 |
| Storage Holds | 10 | 30 |

The generic example ship described above, when purchased new, comes equipped with a maximum weapons energy capacity of 75 and a maximum storage hold capacity of 10. By upgrading, the weapons system can be boosted to 250 and the cargo holds expanded to 30.
...
1. The attacker engages their **combat tractor beam (CTB)** and selects the target.
2. The target is alerted and can open a communication channel, attempt to flee or power up their weapons.
3. Every ship has a delay between establishing a CTB lock and their weapons being sufficiently charged to fire. The delay between volleys varies from ship to ship and is affected by the amount of energy expended.
4. Once the lock has been established and weapons systems are ready, the player selects how much energy to expend in the attack, and fires.
5. Having fired, the player waits for their weapons system to recharge; during this time the other player may return fire or attempt to flee.
6. At any time during the engagement either player has the option to break off the attack (i.e. call for a truce), continue firing, or attempt to flee to an adjacent sector.

Multiple ships can engage a single ship in combat. In that case, the chaos created by the multi-ship attack will increase the odds for the player under assault to successfully flee from the battle. If multiple ships attack a single target and destroy it, all are affected by the damage delivered if the destroyed ship is equipped with Rache devices.

##### 3.6.4.1 Attack and Defense Ratios

Each ship has its own unique attack and defense ratios, which affect how much damage one ship does against another during combat. These ratios may also vary depending on the situation (e.g. planetary assault vs. space combat, or defending against planetary cannons vs. mines vs. standard combat vs. multi-ship engagement).

**Example Ships:**

| Ship | Attack Ratio | Defense Ratio | Weapon Level |
|:---|:---:|:---:|:---:|
| Battleship | 2:1 | 1:1 | 100 |
| Runabout | 1:1 | 2:1 | 40 |

**Scenario A:** If the runabout attacks the battleship with weapons set to maximum, the battleship would lose approximately 40 units of weapon energy (leaving 60 remaining). The runabout would be left defenseless.

**Scenario B:** The battleship (attack ratio 2:1) attacks the runabout (defense ratio 2:1) with weapons set to 20%. The attack/defense ratio would be 1:1. The battleship would inflict approximately 20 units of damage, leaving the runabout with 20 units of power.

**Scenario C:** The battleship attacks the runabout with weapons set to 60% power, exceeding the defensive capabilities of the runabout. The runabout would be disabled or destroyed.

##### 3.6.4.2 Weapons and Shield System Power

A player has three potential sources (and by extension storage locations) for cyrillium/power on their ship:

1. **Weapons system power banks** (refined cyrillium)
2. **Engine power banks** (refined cyrillium)
3. **Cargo holds** (unrefined cyrillium only)

When a player attacks, it drains the weapons system power banks. When fully depleted, the player must visit an outpost to refuel. Cyrillium from the engines cannot be rerouted to the weapons system, but will be diverted to support the ship's shields if under attack.

When defending, the shielding system absorbs incoming weapons fire by automatically pulling energy first from the weapons system until depleted, and then from the engine power. When the shields of a ship are drained, it is in extreme danger of being disabled/captured or destroyed.

##### 3.6.4.3 DodgePods

Every ship has a DodgePod, and if the ship the player is piloting is sufficiently damaged or destroyed (for any reason) the player will eject automatically in a DodgePod. DodgePods have no attack or defensive capabilities.

##### 3.6.4.4 Capturing Vessels

If a player can force an adversary to abandon their ship in combat, the victorious player can capture the abandoned ship. Any ship with towing capability can tow any other ship, though travel while towing will consume more cyrillium.
...
Syndicates can have their own governance models and charters. When a syndicate is formed the founding members may choose to create a charter which outlines the rules and regulations of their syndicate. This allows syndicates to become self-governed entities and to establish how members interact with one another, enforce rules, penalties, share/distribute resources and more.

---

## 4. Cosmic Horizon Economics

Cosmic Horizon economics are structured to be robust and dynamic. The game operates on a play-and-earn (P&E) model, whereby players will strategize, trade, colonize and battle to control strategic sectors of the cosmos -- and earn rewards along the way in the form of $COHO tokens.

### 4.1 In-Game Economy (IGE) Bootstrapping Pool

An important component of Cosmic Horizon's economics is its IGE Bootstrapping Pool, which is leveraged to balance and support in-game economics and outpost trading mechanics. At genesis, a set amount of $COHO is allocated to initially fund the treasury of the various outposts across the galaxy.

### 4.2 Game Tick Economic Support

A periodic game tick injects funds into outpost treasuries, ensuring that outposts maintain sufficient $COHO to purchase commodities from players. This mechanism keeps the economy flowing and prevents outposts from becoming insolvent due to heavy trading activity.

### 4.3 Supply and Demand Pricing

Outpost commodity prices are driven by supply and demand. Outposts with low inventory of a commodity will pay higher prices for it, while outposts flush with a commodity will offer lower prices. This creates natural arbitrage opportunities across the galaxy and rewards players who discover and exploit price differentials.

---

## 5. Ecology

As a promise to planetary regeneration in the real world, Cosmic Horizon's terraforming mechanism will enable players to terraform in-game planets.

As players discover planets they will note that different planet types yield commodities at different rates (see [Planet Types, 3.3.3.1](#3331-planet-types)). Because of this variation, it may be strategically advantageous to have specific combinations of different planet types arranged in their fortified sectors of space.

By taking advantage of this terraforming option players will be able to change the class of a planet to one that has a more strategic output to meet in-game needs.

---

## 6. Perpetual World Mechanics

Cosmic Horizon operates as a perpetual, open-world game rather than a session-based game with fixed end conditions. The following mechanics ensure the galaxy remains dynamic, competitive, and accessible to new players over time.

### 6.1 Regenerating Energy (Action Points)

Players have an energy pool that governs how many actions they can take. Energy regenerates at a rate of **1 action point (AP) per minute**, with a maximum cap of **500 AP**. Most actions consume AP:

| Action | AP Cost |
|:---|:---:|
| Moving between sectors | 1 AP |
| Trading at an outpost | 1 AP |
| Firing a combat volley | 2 AP |
| Deploying items | 1 AP |
| Planetary management | Free |

This system encourages regular play sessions without punishing players who miss a day, and works naturally with the companion app for check-in style play.

### 6.2 Decay and Entropy

To prevent stagnation and ensure the galaxy remains contestable, the following decay mechanics are in effect:

- **Planet population decay:** Unattended planets lose 1-2% of their colonist population per day if the owning player has not logged in within 48 hours.
- **Defense energy drain:** Planetary defenses (shields, atmospheric cannons, PSDs) slowly drain energy over time and must be periodically recharged with refined cyrillium.
- **Deployable degradation:** Deployed drones and mines degrade over time and will self-destruct after approximately 7 days without maintenance from the deploying player.
- **Economic drift:** Outpost commodity prices and inventories drift over time even without player trades, keeping the economy dynamic and preventing price stagnation.

### 6.3 New Player Onboarding

New players begin at a randomly selected Star Mall equipped with:

- A starter ship (modest cargo capacity, minimal weapons)
- Sufficient $COHO for initial trades
- A temporary energy regeneration bonus (2x regeneration rate for their first 72 hours of play)

This gives newcomers a window to explore, learn the mechanics, and establish themselves before competing on even terms with established players.

### 6.4 Frontier Expansion

To keep the galaxy fresh and provide opportunities for late-joining players, new quadrants can be generated and connected to the existing galaxy through newly discovered wormholes or sector connections. These frontier expansions introduce unclaimed planets, new outposts, and unexplored territory, ensuring that established empires do not permanently lock out new players from meaningful progression.

---

## 7. Android Companion App

Cosmic Horizon will feature an Android companion app to extend gameplay beyond the primary web client.

### 7.1 Phase 1: Lite Companion

The initial companion app provides monitoring and light management capabilities:

- View current ship status, location, and inventory
- Check planet production rates and colonist populations
- Monitor commodity prices at discovered outposts
- Receive push notifications for critical events (incoming attacks, completed trades, planet population milestones, drone/mine destruction)
- Quick actions: adjust planet defense settings, configure drone modes
- The companion app connects to the same backend API and WebSocket infrastructure as the web client

### 7.2 Phase 2: Full Mobile Client

In a subsequent phase, the companion app will be expanded into a full game client, allowing players to perform all gameplay actions from their mobile device with a mobile-optimized interface.

---

## 8. Technology

Cosmic Horizon is built as a web-based multiplayer game using modern web technologies.

### 8.1 Backend

The game server is built with Node.js and TypeScript, using Express for the REST API and Socket.io for real-time WebSocket communication. All game logic runs server-side to prevent cheating and ensure consistency across clients. PostgreSQL serves as the primary database, storing the universe graph, player state, economic data, and game logs.
...
The core game engine is isolated from the transport layer, processing all game actions through validated server-side functions. A periodic game tick (every 60 seconds) handles energy regeneration, planetary production, colonist population dynamics, decay mechanics, and outpost economy drift. The engine is designed to be client-agnostic, enabling both the web client and Android companion app to interact through the same API.

### 8.4 Infrastructure

The application is containerized with Docker for consistent deployment. The architecture is monolithic for initial deployment simplicity but internally structured with clean separation between the game engine, API layer, and WebSocket layer to support future extraction into independent services if scaling demands require it.

### 8.5 Future: Blockchain Integration

The architecture is designed to accommodate future blockchain integration. The in-game economy ($COHO) can be extended to support on-chain token mechanics, and game assets (ships, planets) are structured to support NFT representation if desired. This integration is deferred to focus on delivering a polished core gameplay experience first.
