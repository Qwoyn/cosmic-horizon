...
router.post('/garage/store', requireAuth, async (req, res) => {
  try {
    ...
    if (otherShips) {
      await db('players').where({ id: player!.id }).update({ current_ship_id: otherShips.id });
      res.json({ stored: ship.id, switchedTo: otherShips.id });
    } else {
...
router.post('/garage/retrieve/:shipId', requireAuth, async (req, res) => {
  try {
    ...
    const ship = await db('ships').where({ id: shipId, owner_id: player!.id }).first();
    ...
    await db('ships').where({ id: ship.id }).update({ sector_id: player!.current_sector_id });
...
router.get('/garage', requireAuth, async (req, res) => {
  try {
    ...
    const storedShips = await db('ships')
      ...
      .whereNull('sector_id')
      .select('id', 'ship_type_id', 'weapon_energy', 'engine_energy', 'cargo_holds');
...
router.post('/salvage/sell/:shipId', requireAuth, async (req, res) => {
  try {
    ...
    if (shipId === player!.current_ship_id) {
      return res.status(400).json({ error: 'Cannot salvage your active ship. Switch to another first.' });
    }

    const ship = await db('ships').where({ id: shipId, owner_id: player!.id }).first();
    if (!ship) return res.status(404).json({ error: 'Ship not found or not owned by you' });
    if (ship.ship_type_id === 'dodge_pod') {
      return res.status(400).json({ error: 'Dodge pods have no salvage value' });
    }
    ...
    const totalCargo = (ship.cyrillium_cargo || 0) + (ship.food_cargo || 0) +
      (ship.tech_cargo || 0) + (ship.colonist_cargo || 0);
    if (totalCargo > 0) {
      return res.status(400).json({ error: 'Ship must be empty of cargo before salvaging' });
    }
...
router.get('/salvage', requireAuth, async (req, res) => {
  try {
    ...
    const ships = await db('ships')
      ...
      .select('id', 'ship_type_id', 'cyrillium_cargo', 'food_cargo', 'tech_cargo', 'colonist_cargo');

    const salvageOptions = ships
      .filter(s => s.ship_type_id !== 'dodge_pod')
      .map(s => {
        const shipType = SHIP_TYPES.find(st => st.id === s.ship_type_id);
        const totalCargo = (s.cyrillium_cargo || 0) + (s.food_cargo || 0) +
          (s.tech_cargo || 0) + (s.colonist_cargo || 0);
        return {
          id: s.id,
          type: s.ship_type_id,
          name: shipType?.name ?? s.ship_type_id,
...
router.post('/cantina/intel', requireAuth, async (req, res) => {
  try {
    ...
    const topPlanets = await db('planets')
      ...
      .limit(5)
      .select('name', 'sector_id as sectorId', 'colonists', 'planet_class as planetClass');
