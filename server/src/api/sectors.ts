...
router.get('/:id/info', requireAuth, async (req, res) => {
  try {
    ...
    if (sector.claimed_by_player_id) {
      const player = await db('players').where({ id: sector.claimed_by_player_id }).first();
      if (player) owner = { name: player.username, type: 'player' };
    } else if (sector.claimed_by_syndicate_id) {
      const syndicate = await db('syndicates').where({ id: sector.claimed_by_syndicate_id }).first();
...
router.post('/:id/claim', requireAuth, async (req, res) => {
  try {
    ...
    if (player.current_sector_id !== sectorId) {
      return res.status(400).json({ error: 'You must be in the sector to claim it' });
    }

    const sector = await db('sectors').where({ id: sectorId }).first();
    if (!sector) return res.status(404).json({ error: 'Sector not found' });
    if (sector.is_npc_starmall) {
      return res.status(400).json({ error: 'NPC Star Mall sectors cannot be claimed' });
    }
...
router.post('/:id/name', requireAuth, async (req, res) => {
  try {
    ...
    if (sector.claimed_by_player_id === playerId) {
      ...
      const membership = await db('syndicate_members')
        .where({ player_id: playerId, syndicate_id: sector.claimed_by_syndicate_id })
        .first();
      if (!membership || (membership.role !== 'leader' && membership.role !== 'officer')) {
        return res.status(403).json({ error: 'Only syndicate officers can name syndicate sectors' });
      }
    } else {
...
router.post('/:id/conquer', requireAuth, async (req, res) => {
  try {
    ...
    if (!player) return res.status(404).json({ error: 'Player not found' });
    if (player.current_sector_id !== sectorId) {
      return res.status(400).json({ error: 'You must be in the sector to conquer it' });
    }

    const sector = await db('sectors').where({ id: sectorId }).first();
    if (!sector) return res.status(404).json({ error: 'Sector not found' });
    if (sector.is_npc_starmall) {
      return res.status(400).json({ error: 'NPC Star Mall sectors cannot be conquered' });
    }
    if (!sector.claimed_by_player_id && !sector.claimed_by_syndicate_id) {
      return res.status(400).json({ error: 'Sector is not claimed — use claim instead' });
    }

    // Check if player owns majority of planets in sector
    const totalPlanets = await db('planets').where({ sector_id: sectorId }).count('id as count').first();
    const ownedPlanets = await db('planets').where({ sector_id: sectorId, owner_id: player.id }).count('id as count').first();
    const total = Number(totalPlanets?.count || 0);
    const owned = Number(ownedPlanets?.count || 0);

    if (total === 0 || owned <= total / 2) {
      return res.status(400).json({ error: 'You must own a majority of planets in the sector to conquer it' });
    }
    ...
    if (claimType === 'syndicate') {
      ...
      if (!membership) return res.status(400).json({ error: 'You are not in a syndicate' });
      await db('sectors').where({ id: sectorId }).update({
