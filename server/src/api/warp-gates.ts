...
router.get('/sector', requireAuth, async (req, res) => {
  try {
    ...
    const gates = await db('warp_gates')
      .where({ status: 'active' })
      .where(function() {
        this.where({ sector_a_id: player.current_sector_id })
          .orWhere({ sector_b_id: player.current_sector_id });
      });

    res.json({
      gates: gates.map(g => ({
        id: g.id,
        destinationSectorId: g.sector_a_id === player.current_sector_id ? g.sector_b_id : g.sector_a_id,
...
router.post('/build', requireAuth, async (req, res) => {
  try {
    ...
    if (!destinationSectorId) return res.status(400).json({ error: 'Missing destination sector' });
    ...
    if (!membership) return res.status(400).json({ error: 'Must be in a syndicate' });
    if (!['leader', 'officer'].includes(membership.role)) {
      return res.status(400).json({ error: 'Must be syndicate officer or leader' });
    }
    ...
    if ((ship.tech_cargo || 0) < GAME_CONFIG.WARP_GATE_COST_TECH) {
      return res.status(400).json({ error: `Need ${GAME_CONFIG.WARP_GATE_COST_TECH} tech in cargo` });
    }
    if ((ship.cyrillium_cargo || 0) < GAME_CONFIG.WARP_GATE_COST_CYRILLIUM) {
      return res.status(400).json({ error: `Need ${GAME_CONFIG.WARP_GATE_COST_CYRILLIUM} cyrillium in cargo` });
    }
    ...
    await db('ships').where({ id: ship.id }).update({
      ...
      cyrillium_cargo: (ship.cyrillium_cargo || 0) - GAME_CONFIG.WARP_GATE_COST_CYRILLIUM,
    });
...
router.post('/use/:gateId', requireAuth, async (req, res) => {
  try {
    ...
    if (gate.sector_a_id !== player.current_sector_id && gate.sector_b_id !== player.current_sector_id) {
      return res.status(400).json({ error: 'Not at this warp gate' });
    }

    const destinationSectorId = gate.sector_a_id === player.current_sector_id
      ? gate.sector_b_id : gate.sector_a_id;
    ...
    const toll = calculateToll(gate.toll_amount, !!membership, !!gate.syndicate_free);
...
router.post('/set-toll/:gateId', requireAuth, async (req, res) => {
  try {
    ...
    const gate = await db('warp_gates').where({ id: req.params.gateId, status: 'active' }).first();
    if (!gate) return res.status(404).json({ error: 'Warp gate not found' });

    // Must be syndicate officer+
    const membership = await db('syndicate_members')
      .where({ player_id: req.session.playerId, syndicate_id: gate.syndicate_id })
      .first();

    if (!membership || !['leader', 'officer'].includes(membership.role)) {
      return res.status(403).json({ error: 'Must be syndicate officer or leader' });
    }
...
router.get('/syndicate', requireAuth, async (req, res) => {
  try {
    ...
    if (!membership) return res.status(400).json({ error: 'Not in a syndicate' });

    const gates = await db('warp_gates')
      .where({ syndicate_id: membership.syndicate_id, status: 'active' });
