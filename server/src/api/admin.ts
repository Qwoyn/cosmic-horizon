...
router.get('/economy/snapshot', async (_req: Request, res: Response) => {
  try {
    ...
    const outposts = await db('outposts').select(
      'id', 'name', 'treasury',
      'cyrillium_stock', 'cyrillium_capacity',
      'food_stock', 'food_capacity',
      'tech_stock', 'tech_capacity',
    );
    ...
    const outpostData = outposts.map((o: any) => ({
      ...
      treasury: o.treasury,
      prices: {
        cyrillium: calculatePrice('cyrillium', o.cyrillium_stock, o.cyrillium_capacity),
        food: calculatePrice('food', o.food_stock, o.food_capacity),
        tech: calculatePrice('tech', o.tech_stock, o.tech_capacity),
      },
      stocks: {
        cyrillium: { current: o.cyrillium_stock, capacity: o.cyrillium_capacity },
...
router.get('/config/overrides', async (_req: Request, res: Response) => {
  try {
    ...
    const rows = await db('game_config_overrides').select('key', 'value', 'updated_at');
    res.json(rows);
  } catch (err: any) {
    res.status(500).json({ error: err.message });
  }
...
router.post('/config/overrides', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (existing) {
      await db('game_config_overrides').where({ key }).update({
        value: String(value),
