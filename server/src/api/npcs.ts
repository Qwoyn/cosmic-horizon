...
import { getVendorItemsForNPC, getAllVendorItems } from '../config/vendor-items';
...
router.get('/:npcId/vendor', requireAuth, async (req, res) => {
  try {
    ...
    const services = typeof npc.services === 'string' ? JSON.parse(npc.services) : npc.services;
    ...
    const vendorItems = items.map(item => ({
      ...
      available: (fameMap.get(item.requiredFactionId) || 0) >= item.requiredFame,
    }));
...
router.post('/:npcId/vendor/buy', requireAuth, async (req, res) => {
  try {
    ...
    if (!item) return res.status(404).json({ error: 'Item not found at this vendor' });
    ...
    if (playerFame < item.requiredFame) {
      return res.status(400).json({ error: `Requires ${item.requiredFame} fame with ${item.requiredFactionId} (you have ${playerFame})` });
    }

    // Check credits
    if (Number(player.credits) < item.price) {
      return res.status(400).json({ error: `Not enough credits (need ${item.price}, have ${player.credits})` });
    }

    // Deduct credits and add to inventory (stored as game_events)
    await db('players').where({ id: player.id }).decrement('credits', item.price);

    await db('game_events').insert({
      id: require('crypto').randomUUID(),
      player_id: player.id,
      event_type: `item:${itemId}`,
      data: JSON.stringify({ itemId, purchasedAt: new Date(), source: 'npc_vendor', npcId: req.params.npcId }),
      read: false,
    });

    res.json({ success: true, message: `Purchased ${item.name}`, creditsSpent: item.price });
  } catch (err) {
