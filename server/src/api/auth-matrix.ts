 ...
 */
async function validateMatrixToken(token: MatrixOpenIDToken): Promise<string | null> {
  try {
    // The matrix_server_name tells us which homeserver to validate against.
    // Construct the userinfo URL per Matrix spec.
    const serverName = token.matrix_server_name;
    const url = `https://${serverName}/_matrix/federation/v1/openid/userinfo?access_token=${encodeURIComponent(token.access_token)}`;
...
router.post('/', async (req, res) => {
  try {
    ...
    const existing = await db('players').where({ matrix_user_id: matrixUserId }).first();
    if (existing) {
      // Existing player — log them in
      await db('players').where({ id: existing.id }).update({ last_login: new Date() });
    ...
    const race = (requestedRace && VALID_RACE_IDS.includes(requestedRace))
      ? requestedRace as RaceId
      : VALID_RACE_IDS[Math.floor(Math.random() * VALID_RACE_IDS.length)] as RaceId;
    ...
    await db('ships').insert({
      ...
      weapon_energy: shipTypeConfig.baseWeaponEnergy + raceConfig.starterWeaponBonus,
      max_weapon_energy: shipTypeConfig.maxWeaponEnergy,
      engine_energy: shipTypeConfig.baseEngineEnergy + raceConfig.starterEngineBonus,
    ...
    if (err.message?.includes('UNIQUE constraint failed: players.matrix_user_id')) {
...
router.post('/link', requireAuth, async (req, res) => {
  try {
    ...
    const conflict = await db('players')
      ...
      .first();
    if (conflict) {
      return res.status(409).json({ error: 'Matrix account already linked to another player' });
    }

    // Check if this player already has a Matrix account linked
    const player = await db('players').where({ id: playerId }).first();
    if (player?.matrix_user_id) {
      return res.status(400).json({ error: 'Player already has a Matrix account linked. Unlink first.' });
    }
...
router.post('/unlink', requireAuth, async (req, res) => {
  try {
    ...
    if (player.password_hash?.startsWith('matrix:')) {
      return res.status(400).json({ error: 'Cannot unlink Matrix from a Matrix-only account. Set a password first.' });
    }
