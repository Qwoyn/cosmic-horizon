...
router.get('/inbox', requireAuth, async (req, res) => {
  try {
    const messages = await db('messages')
      ...
      .where({ 'messages.recipient_id': req.session.playerId, 'messages.recipient_deleted': false })
      .orderBy('messages.created_at', 'desc')
      .limit(GAME_CONFIG.MAX_MESSAGES_PER_PLAYER)
      .select(
        'messages.id', 'messages.subject', 'messages.read',
...
router.get('/sent', requireAuth, async (req, res) => {
  try {
    const messages = await db('messages')
      ...
      .where({ 'messages.sender_id': req.session.playerId, 'messages.sender_deleted': false })
      .orderBy('messages.created_at', 'desc')
      .limit(GAME_CONFIG.MAX_MESSAGES_PER_PLAYER)
      .select(
        'messages.id', 'messages.subject',
        'messages.created_at as createdAt',
...
router.get('/:id', requireAuth, async (req, res) => {
  try {
    const message = await db('messages')
      ...
      .where(function() {
        this.where({ sender_id: req.session.playerId })
          .orWhere({ recipient_id: req.session.playerId });
      })
      .first();
    ...
    const sender = await db('players').where({ id: message.sender_id }).select('username').first();
    const recipient = await db('players').where({ id: message.recipient_id }).select('username').first();
...
router.post('/send', requireAuth, async (req, res) => {
  try {
    ...
    if (body.length > GAME_CONFIG.MAX_MESSAGE_BODY_LENGTH) {
      return res.status(400).json({ error: `Message body too long (max ${GAME_CONFIG.MAX_MESSAGE_BODY_LENGTH} characters)` });
    }
...
router.delete('/:id', requireAuth, async (req, res) => {
  try {
    ...
    if (message.sender_id === req.session.playerId) {
      await db('messages').where({ id: message.id }).update({ sender_deleted: true });
    } else if (message.recipient_id === req.session.playerId) {
      await db('messages').where({ id: message.id }).update({ recipient_deleted: true });
    } else {
...
router.get('/unread-count', requireAuth, async (req, res) => {
  try {
    const result = await db('messages')
      .where({ recipient_id: req.session.playerId, read: false, recipient_deleted: false })
      .count('* as count')
      .first();
