...
router.get('/available', requireAuth, async (req, res) => {
  try {
    ...
    const pool = await generateMissionPool(player.id, player.current_sector_id, playerLevel);

    // Add lock info for prerequisite missions
    const enriched = await Promise.all(pool.map(async (m: any) => {
      let prerequisiteMet = true;
      if (m.prerequisiteMissionId) {
        prerequisiteMet = await checkPrerequisite(player.id, m.prerequisiteMissionId);
      }
    ...
    const lockedTiers = Object.entries(tierLevels)
      ...
      .map(([tier, reqLevel]) => ({ tier: Number(tier), requiredLevel: reqLevel }));
...
router.post('/accept/:templateId', requireAuth, async (req, res) => {
  try {
    ...
    if (playerLevel < requiredLevel) {
      return res.status(400).json({ error: `Requires level ${requiredLevel} to accept tier ${template.tier} missions` });
    }

    // Check prerequisite
    if (template.prerequisite_mission_id) {
      const met = await checkPrerequisite(player.id, template.prerequisite_mission_id);
      if (!met) {
        return res.status(400).json({ error: 'Prerequisite mission not yet completed' });
      }
    }
    ...
    const objectives = typeof template.objectives === 'string' ? JSON.parse(template.objectives) : template.objectives;
...
router.get('/active', requireAuth, async (req, res) => {
  try {
    ...
    const pendingClaim = await db('player_missions')
      .join('mission_templates', 'player_missions.template_id', 'mission_templates.id')
      .where({ 'player_missions.player_id': req.session.playerId, 'player_missions.status': 'completed', 'player_missions.claim_status': 'pending_claim' })
      .select(
        'player_missions.id as missionId',
        'mission_templates.title',
        'mission_templates.description',
    ...
    res.json({
      missions: allMissions.map(m => ({
        ...
        objectivesDetail: typeof m.objectivesDetail === 'string' ? JSON.parse(m.objectivesDetail) : (m.objectivesDetail || []),
...
router.get('/claimable', requireAuth, async (req, res) => {
  try {
    ...
    const sector = await db('sectors').where({ id: player.current_sector_id }).first();
    if (!sector?.has_star_mall) {
      return res.status(400).json({ error: 'Must be at a star mall to claim rewards' });
    }

    const missions = await db('player_missions')
      .join('mission_templates', 'player_missions.template_id', 'mission_templates.id')
      .where({ 'player_missions.player_id': player.id, 'player_missions.status': 'completed', 'player_missions.claim_status': 'pending_claim' })
      .select(
        'player_missions.id as missionId',
        'mission_templates.title',
        'mission_templates.tier',
...
router.post('/claim/:missionId', requireAuth, async (req, res) => {
  try {
    ...
    if (!mission) {
      return res.status(404).json({ error: 'No claimable mission found with that ID' });
    }
