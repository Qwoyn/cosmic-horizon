...
router.post('/deploy', requireAuth, async (req, res) => {
  try {
    ...
    if (sector?.type === 'protected') {
      return res.status(400).json({ error: 'Cannot deploy in protected sectors' });
    }
    ...
    await db('deployables').insert({
      ...
      power_level: 1,
      toll_amount: item.deployableType === 'drone_toll' ? (tollAmount ?? 100) : null,
...
router.get('/mine', requireAuth, async (req, res) => {
  try {
    const deployables = await db('deployables')
      ...
      .select(
        'id', 'type', 'sector_id as sectorId',
        'power_level as powerLevel', 'health',
...
router.delete('/:id', requireAuth, async (req, res) => {
  try {
    const deployable = await db('deployables').where({ id: req.params.id }).first();
    if (!deployable) return res.status(404).json({ error: 'Deployable not found' });
...
router.post('/:id/maintain', requireAuth, async (req, res) => {
  try {
    ...
    if (player.current_sector_id !== deployable.sector_id) {
      return res.status(400).json({ error: 'Must be in same sector to maintain' });
    }
