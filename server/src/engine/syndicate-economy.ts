...
export async function getMemberSyndicate(playerId: string): Promise<{ syndicateId: string; role: string }> {
...
export async function getPoolResources(syndicateId: string) {
  const resources = await db('syndicate_resource_pool')
    .join('resource_definitions', 'syndicate_resource_pool.resource_id', 'resource_definitions.id')
...
export async function depositToPool(playerId: string, resourceId: string, quantity: number) {
  ...
  if (level === 'none') throw new Error('You do not have permission to deposit to the pool');
  ...
  if (!playerRes || playerRes.quantity < quantity) {
    throw new Error(`Not enough resources (have ${playerRes?.quantity || 0}, need ${quantity})`);
  }
  ...
  // Log
  const resDef = await db('resource_definitions').where({ id: resourceId }).first();
...
export async function withdrawFromPool(playerId: string, resourceId: string, quantity: number) {
  if (quantity <= 0) throw new Error('Quantity must be positive');
  ...
  if (!poolRes || poolRes.quantity < quantity) {
    throw new Error(`Not enough in pool (have ${poolRes?.quantity || 0}, need ${quantity})`);
  }
...
export async function setPoolPermission(setterId: string, targetPlayerId: string, level: string) {
  const validLevels = ['none', 'deposit', 'full', 'manager'];
  if (!validLevels.includes(level)) {
    throw new Error(`Invalid permission level. Must be one of: ${validLevels.join(', ')}`);
  }
  ...
  if (role !== 'leader' && role !== 'officer') {
    ...
    if (setterPerm?.level !== 'manager') {
      throw new Error('Only leaders, officers, or pool managers can set permissions');
    }
  }
...
export async function getPoolLog(syndicateId: string, limit: number = 50) {
  return db('syndicate_pool_log')
    ...
    .leftJoin('resource_definitions', 'syndicate_pool_log.resource_id', 'resource_definitions.id')
...
export async function designateFactory(playerId: string, planetId: string) {
  ...
  if (planet.owner_id !== playerId) throw new Error('You do not own this planet');
  if (planet.upgrade_level < GAME_CONFIG.SYNDICATE_FACTORY_MIN_LEVEL) {
    throw new Error(`Planet must be level ${GAME_CONFIG.SYNDICATE_FACTORY_MIN_LEVEL} or higher`);
  }
  if (planet.is_syndicate_factory) throw new Error('This planet is already a factory');
  ...
  if (Number(factoryCount?.count || 0) >= GAME_CONFIG.SYNDICATE_MAX_FACTORIES) {
    throw new Error(`Syndicate already has the maximum number of factories (${GAME_CONFIG.SYNDICATE_MAX_FACTORIES})`);
  }

  // Check treasury
  const syndicate = await db('syndicates').where({ id: syndicateId }).first();
  if (!syndicate || Number(syndicate.treasury) < GAME_CONFIG.SYNDICATE_FACTORY_COST) {
    throw new Error(`Not enough in treasury (need ${GAME_CONFIG.SYNDICATE_FACTORY_COST.toLocaleString()} credits)`);
  }
...
export async function revokeFactory(playerId: string) {
  const { syndicateId, role } = await getMemberSyndicate(playerId);
  if (role !== 'leader') {
    throw new Error('Only the syndicate leader can revoke a factory designation');
  }
  ...
  // Clear whitelist
  await db('syndicate_factory_whitelist').where({ planet_id: factory.id }).del();
...
export async function processFactoryProduction(planet: any) {
  ...
  for (const commodity of commodities) {
    ...
    const existing = await db('syndicate_resource_pool')
      .where({ syndicate_id: planet.factory_syndicate_id, resource_id: commodity.resourceId })
      .first();
    if (existing) {
      await db('syndicate_resource_pool')
        .where({ syndicate_id: planet.factory_syndicate_id, resource_id: commodity.resourceId })
...
export async function startProject(playerId: string, projectTypeId: string, targetSectorId?: number) {
  const { syndicateId, role } = await getMemberSyndicate(playerId);
  if (role !== 'leader') {
  ...
  const definition = await db('mega_project_definitions').where({ id: projectTypeId }).first();
  ...
  if (Number(memberCount?.count || 0) < definition.min_syndicate_members) {
    throw new Error(`Need at least ${definition.min_syndicate_members} members to start this project`);
  }
  ...
  const activeCount = await db('syndicate_projects')
    ...
    .count('id as count')
    .first();
  if (Number(activeCount?.count || 0) >= GAME_CONFIG.SYNDICATE_MAX_ACTIVE_PROJECTS) {
    throw new Error(`Cannot have more than ${GAME_CONFIG.SYNDICATE_MAX_ACTIVE_PROJECTS} active projects`);
  }
...
export async function contributeToProject(
  ...
  if (project.status !== 'in_progress') throw new Error('Project is not accepting contributions');

  const definition = await db('mega_project_definitions')
    .where({ id: project.project_type }).first();
  ...
  if (resourceId === null) {
    ...
    await db('syndicate_projects').where({ id: projectId })
    ...
    const requirements = JSON.parse(definition.resource_requirements) as { resourceId: string; quantity: number }[];
    ...
    const contributed = JSON.parse(project.resources_contributed) as Record<string, number>;
    const alreadyContributed = contributed[resourceId] || 0;
    const remaining = req.quantity - alreadyContributed;
    const actual = Math.min(quantity, remaining);
    if (actual <= 0) throw new Error('This resource requirement is already met');
    ...
    if (fromPool) {
      ...
      if (permLevel !== 'full' && permLevel !== 'manager') {
        throw new Error('You need full or manager pool access to contribute from pool');
      }
      ...
      if (!playerRes || playerRes.quantity < actual) {
        throw new Error(`Not enough resources (have ${playerRes?.quantity || 0})`);
      }
      await db('player_resources')
        .where({ player_id: playerId, resource_id: resourceId })
        .decrement('quantity', actual);
    ...
    return { contributed: actual, type: 'resource', resourceName: resDef?.name || resourceId };
  }
}

async function checkProjectFullyFunded(projectId: string) {
  const project = await db('syndicate_projects').where({ id: projectId }).first();
...
export async function checkAndCompleteProjects() {
  ...
  for (const project of buildingProjects) {
    ...
    const buildEnd = new Date(buildStarted.getTime() + definition.build_time_hours * 60 * 60 * 1000);
...
export async function getProjectDetail(projectId: string) {
  ...
  const contributions = await db('syndicate_project_contributions')
    ...
    .leftJoin('resource_definitions', 'syndicate_project_contributions.resource_id', 'resource_definitions.id')
  ...
  for (const req of requirements) {
    const resDef = await db('resource_definitions').where({ id: req.resourceId }).first();
  ...
  if (project.status === 'building' && project.build_started_at) {
    const buildStarted = new Date(project.build_started_at);
    const buildEnd = new Date(buildStarted.getTime() + definition.build_time_hours * 60 * 60 * 1000);
    const elapsed = (new Date().getTime() - buildStarted.getTime()) / (1000 * 60 * 60);
    buildProgress = {
...
export async function cancelProject(playerId: string, projectId: string) {
  ...
  const project = await db('syndicate_projects')
    ...
    .first();
  if (!project) throw new Error('Project not found');
  if (project.status === 'completed') throw new Error('Cannot cancel a completed project');
  ...
  return { refundedResources: contributed, refundedCredits: project.credits_contributed };
}
...
export async function getSyndicateProjects(syndicateId: string) {
  const projects = await db('syndicate_projects')
    .join('mega_project_definitions', 'syndicate_projects.project_type', 'mega_project_definitions.id')
  ...
  return projects.map(p => {
    const requirements = JSON.parse(p.resourceRequirements) as { resourceId: string; quantity: number }[];
    const contributed = JSON.parse(p.resourcesContributed) as Record<string, number>;
    const totalRequired = requirements.reduce((sum, r) => sum + r.quantity, 0);
    const totalContributed = requirements.reduce((sum, r) => sum + (contributed[r.resourceId] || 0), 0);
    const creditsPercent = p.creditsRequired > 0 ? Math.floor((p.creditsContributed / p.creditsRequired) * 100) : 100;
    const resourcesPercent = totalRequired > 0 ? Math.floor((totalContributed / totalRequired) * 100) : 100;
