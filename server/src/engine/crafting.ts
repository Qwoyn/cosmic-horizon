...
export async function getPlayerResources(playerId: string) {
  return db('player_resources')
    .join('resource_definitions', 'player_resources.resource_id', 'resource_definitions.id')
...
export async function getAvailableRecipes(planetLevel: number) {
  ...
  for (const recipe of recipes) {
    const ingredients = await db('recipe_ingredients')
      .join('resource_definitions', 'recipe_ingredients.resource_id', 'resource_definitions.id')
...
export async function startCraft(
  ...
  if (batchSize < 1 || batchSize > GAME_CONFIG.CRAFTING_MAX_BATCH_SIZE) {
    throw new Error(`Batch size must be 1-${GAME_CONFIG.CRAFTING_MAX_BATCH_SIZE}`);
  }
  ...
  if (planet.upgrade_level < recipe.planet_level_required) {
    throw new Error(`Planet must be level ${recipe.planet_level_required} for this recipe`);
  }
  ...
  for (const ing of ingredients) {
    ...
    if (available < needed) {
      const resDef = await db('resource_definitions').where({ id: ing.resource_id }).first();
      throw new Error(`Not enough ${resDef?.name || ing.resource_id} (need ${needed}, have ${available})`);
    }
  }
  ...
  if (totalCredits > 0) {
    ...
    await db('players').where({ id: playerId }).decrement('credits', totalCredits);
  }

  // Award XP
  const xpAmount = recipe.tier === 2 ? GAME_CONFIG.XP_CRAFT_TIER2
  ...
  if (recipe.craft_time_minutes > 0) {
    const now = new Date();
    const completesAt = new Date(now.getTime() + recipe.craft_time_minutes * batchSize * 60 * 1000);
...
async function applyInstantOutput(playerId: string, recipe: any, batchSize: number) {
  ...
  if (recipe.output_item_type === 'resource' && recipe.output_resource_id) {
    ...
    const resDef = await db('resource_definitions').where({ id: recipe.output_resource_id }).first();
    return { type: 'resource', name: resDef?.name || recipe.output_resource_id, quantity: totalQty };
  }
  ...
  if (recipe.output_item_type === 'upgrade') {
    return { type: 'upgrade', upgradeTypeId: recipe.output_item_id, name: recipe.name, quantity: totalQty };
  }

  if (recipe.output_item_type === 'consumable') {
    return { type: 'consumable', itemId: recipe.output_item_id, name: recipe.name, quantity: totalQty };
  }
...
export async function collectRefineryBatch(playerId: string, queueId: string) {
  const entry = await db('planet_refinery_queue')
    ...
    .select('planet_refinery_queue.*', 'recipes.name as recipe_name',
      'recipes.output_resource_id', 'recipes.output_item_type',
      'recipes.output_item_id', 'recipes.output_quantity')
    .first();
  ...
  await db('planet_refinery_queue').where({ id: queueId }).update({ collected: true });

  const output = await applyInstantOutput(playerId, entry, entry.batch_size);
  const xpResult = await awardXP(playerId, GAME_CONFIG.XP_COLLECT_RESOURCES, 'craft');
...
export async function producePlanetUniqueResources(planet: any) {
  ...
  if (!planetType?.uniqueResources || !planet.owner_id || planet.colonists <= 0) return;

  const colonists = planet.colonists || 0;
  const idealPop = planetType.idealPopulation;
  const efficiency = colonists <= idealPop ? 1.0 : Math.max(0.5, 1.0 - (colonists - idealPop) / idealPop);

  for (const res of planetType.uniqueResources) {
    const produced = Math.floor(res.rate * (colonists / 1000) * efficiency * GAME_CONFIG.CRAFTING_UNIQUE_RESOURCE_RATE);
...
export async function collectPlanetResources(playerId: string, planetId: string) {
  const planet = await db('planets').where({ id: planetId }).first();
  if (!planet) throw new Error('Planet not found');
  if (planet.owner_id !== playerId) throw new Error('You do not own this planet');
  ...
  const baseCommodities = [
    { field: 'cyrillium_stock', resourceId: 'cyrillium', name: 'Cyrillium' },
    { field: 'food_stock', resourceId: 'food', name: 'Food' },
    { field: 'tech_stock', resourceId: 'tech', name: 'Tech Components' },
  ];

  for (const commodity of baseCommodities) {
    const amount = planet[commodity.field] || 0;
    if (amount > 0) {
      await adjustPlayerResource(playerId, commodity.resourceId, amount);
      collected.push({ resourceId: commodity.resourceId, name: commodity.name, quantity: amount });
    }
  }
  ...
  const planetResources = await db('planet_resources')
    ...
    .where({ 'planet_resources.planet_id': planetId })
    .where('planet_resources.stock', '>', 0)
    .select('planet_resources.resource_id', 'resource_definitions.name', 'planet_resources.stock');

  for (const pr of planetResources) {
    await adjustPlayerResource(playerId, pr.resource_id, pr.stock);
    collected.push({ resourceId: pr.resource_id, name: pr.name, quantity: pr.stock });
  }

  await db('planet_resources').where({ planet_id: planetId }).update({ stock: 0 });

  const xpResult = await awardXP(playerId, GAME_CONFIG.XP_COLLECT_RESOURCES, 'explore');
...
export async function adjustPlayerResource(playerId: string, resourceId: string, amount: number) {
