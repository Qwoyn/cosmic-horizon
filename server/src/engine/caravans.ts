...
export async function computeRoutePath(
  ...
  const edgeRows = await db('sector_edges')
    .select('from_sector_id', 'to_sector_id', 'one_way');
  ...
  const path = findShortestPath(edgeMap, sourceSectorId, destSectorId, GAME_CONFIG.TRADE_ROUTE_MAX_PATH_LENGTH);
...
export async function dispatchCaravans(io: SocketIOServer): Promise<void> {
  const now = new Date();
  const intervalMs = GAME_CONFIG.TRADE_ROUTE_DISPATCH_INTERVAL_TICKS * GAME_CONFIG.TICK_INTERVAL_MS;
  ...
  for (const route of activeRoutes) {
    ...
    // Deduct credits
    await db('players').where({ id: route.owner_id }).update({
    ...
    if (route.fuel_paid) {
      const fuelResource = await db('player_resources')
        ...
        .first();
      if (fuelResource && fuelResource.quantity >= GAME_CONFIG.TRADE_ROUTE_FUEL_COST) {
        await adjustPlayerResource(route.owner_id, 'drift_fuel', -GAME_CONFIG.TRADE_ROUTE_FUEL_COST);
        isProtected = 1;
      }
 ...
 */
async function handleCaravanArrival(io: SocketIOServer, caravan: any): Promise<void> {
  const now = new Date();
  ...
  // Notify owner
  const planet = await db('planets').where({ id: route.dest_planet_id }).first();
  ...
  logActivity(caravan.owner_id, 'caravan_delivered', `Caravan delivered ${caravan.food_cargo} food to ${planet?.name || 'planet'}`, { foodDelivered: caravan.food_cargo });
...
export async function ransackCaravan(
  ...
  attackerId: string,
  caravanId: string
): Promise<{ success: boolean; error?: string; loot?: any; combatResult?: any }> {
  const caravan = await db('caravans').where({ id: caravanId }).first();
  if (!caravan) return { success: false, error: 'Caravan not found' };
  if (caravan.status !== 'in_transit') return { success: false, error: 'Caravan is not in transit' };

  const attacker = await db('players').where({ id: attackerId }).first();
  if (!attacker) return { success: false, error: 'Player not found' };

  if (caravan.owner_id === attackerId) return { success: false, error: 'Cannot ransack your own caravan' };
  ...
  const sector = await db('sectors').where({ id: caravan.current_sector_id }).first();
  if (sector && (sector.type === 'protected' || sector.type === 'harmony_enforced')) {
    return { success: false, error: 'Combat not allowed in this sector' };
  }

  // Build attacker combat state from ship
  const ship = await db('ships').where({ id: attacker.current_ship_id }).first();
  ...
  const combatResult = resolveCombatVolley(attackerState, defenderState, ship.weapon_energy);
  ...
  const foodLoot = Math.floor(caravan.food_cargo * GAME_CONFIG.CARAVAN_RANSACK_FOOD_PERCENT);
  ...
  // Set pirate flag
  const pirateUntil = new Date(now.getTime() + GAME_CONFIG.PIRATE_COOLDOWN_HOURS * 3600000);
  await db('players').where({ id: attackerId }).update({
    pirate_until: pirateUntil.toISOString(),
  });

  // Destroy caravan
  await db('caravans').where({ id: caravan.id }).update({ status: 'destroyed' });

  // Destroy the trade route
  await db('trade_routes').where({ id: caravan.trade_route_id }).update({ status: 'destroyed' });
  ...
  logActivity(attackerId, 'caravan_ransacked', `Ransacked a caravan — looted ${foodLoot} food and ${creditLoot} credits`, { foodLoot, creditLoot });
  logActivity(caravan.owner_id, 'caravan_lost', `Caravan ransacked by ${attacker.username} in sector ${caravan.current_sector_id}`, { attackerId, sectorId: caravan.current_sector_id });
...
export async function escortCaravan(
  ...
  try {
    escortSyndicate = await db('syndicate_members').where({ player_id: escortPlayerId }).first();
    ownerSyndicate = await db('syndicate_members').where({ player_id: caravan.owner_id }).first();
  } catch { /* table may not exist yet */ }

  if (!escortSyndicate || !ownerSyndicate || escortSyndicate.syndicate_id !== ownerSyndicate.syndicate_id) {
    return { success: false, error: 'Must be in the same syndicate as the caravan owner' };
  }
  ...
  logActivity(escortPlayerId, 'caravan_escorted', `Escorted a caravan in sector ${caravan.current_sector_id}`);
