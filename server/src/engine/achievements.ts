...
export async function checkAchievements(
  ...
  try {
    ...
    const candidates = await db('achievement_definitions')
      .whereNotIn('id', earned.length > 0 ? earned : ['__none__']);
    ...
    for (const def of candidates) {
      ...
      if (met) {
        ...
        if (def.credit_reward > 0) {
          await db('players').where({ id: playerId }).increment('credits', def.credit_reward);
        }
...
async function evaluateAchievement(
  ...
  switch (def.id) {
    // Leveling
    case 'reach_level_10':  return action === 'level_up' && data.newLevel >= 10;
    case 'reach_level_25':  return action === 'level_up' && data.newLevel >= 25;
    case 'reach_level_50':  return action === 'level_up' && data.newLevel >= 50;
    case 'reach_level_75':  return action === 'level_up' && data.newLevel >= 75;
    case 'reach_level_100': return action === 'level_up' && data.newLevel >= 100;

    // Combat
    case 'first_kill': return action === 'combat_destroy';
    case 'destroy_10': {
      if (action !== 'combat_destroy') return false;
      const count = await db('combat_logs').where({ attacker_id: playerId, outcome: 'ship_destroyed' }).count('* as c').first();
    ...
    // Trading
    case 'first_trade': return action === 'trade';
    ...
    // Missions
    case 'first_mission': return action === 'mission_complete';
    case 'complete_10_missions': {
      if (action !== 'mission_complete') return false;
      const count = await db('player_missions').where({ player_id: playerId, status: 'completed' }).count('* as c').first();
      return Number(count?.c || 0) >= 10;
    }
    ...
    // Planets
    case 'first_planet': return action === 'claim_planet';
    case 'own_5_planets': {
      if (action !== 'claim_planet') return false;
      const count = await db('planets').where({ owner_id: playerId }).count('* as c').first();
