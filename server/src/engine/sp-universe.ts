...
import { createRng, generateOutposts, generatePlanets, generateSeedPlanets } from './sp-helpers';
 ...
 */
export async function generateSPUniverse(playerId: string, knex: Knex): Promise<{
  sectorOffset: number;
  ...
  const sectorRows: {
    id: number; type: string; has_star_mall: boolean; has_seed_planet: boolean;
    region_id: number; universe: string; owner_id: string; sp_mall_locked: boolean;
  }[] = [];
  ...
  const starMallSectorIds = sectorRows.filter(s => s.has_star_mall).map(s => s.id);
  const seedPlanetSectorIds = sectorRows.filter(s => s.has_seed_planet).map(s => s.id);

  // 6. Generate and insert outposts
  const rng = createRng(SP_SEED + 100); // Different sub-seed for outpost placement
  const outposts = generateOutposts(allSPSectorIds, starMallSectorIds, rng, SP_NUM_OUTPOSTS);

  // Ensure star mall outposts sell at least one commodity
  for (const outpost of outposts) {
    if (starMallSectorIds.includes(outpost.sector_id)) {
      const sellsSomething = outpost.cyrillium_mode === 'sell' ||
...
async function placeSPNPCs(
  ...
  const SP_NPC_DEFS = [
    { name: 'Vex Ironhide', title: 'Bartender', race: 'muscarian', faction_id: 'traders_guild', services: ['info', 'trade'] },
    { name: 'Mira Dustwalker', title: 'Bartender', race: 'tarri', faction_id: 'independent', services: ['info'] },
    { name: 'Zek Thornspore', title: 'Arms Dealer', race: 'muscarian', faction_id: 'shadow_syndicate', services: ['trade'] },
    { name: 'Lyris Starweaver', title: 'Navigator', race: 'vedic', faction_id: 'cosmic_scholars', services: ['info'] },
    { name: 'Korr Ashborn', title: 'Bounty Hunter', race: 'kalin', faction_id: 'frontier_rangers', services: ['quest'] },
    { name: 'Nym Quicksilver', title: 'Smuggler', race: 'tarri', faction_id: 'shadow_syndicate', services: ['trade', 'info'] },
    { name: 'Sage Voidwhisper', title: 'Scholar', race: 'vedic', faction_id: 'cosmic_scholars', services: ['info'] },
    { name: 'Grak Steelwall', title: 'Mercenary', race: 'kalin', faction_id: 'independent', services: ['quest'] },
    { name: 'Tova Embermoss', title: 'Medic', race: 'muscarian', faction_id: 'frontier_rangers', services: ['info'] },
    { name: 'Xel Driftborne', title: 'Trader', race: 'tarri', faction_id: 'traders_guild', services: ['trade'] },
    { name: 'Oren Glacius', title: 'Miner', race: 'kalin', faction_id: 'independent', services: ['trade'] },
    { name: 'Asha Mindflow', title: 'Mystic', race: 'vedic', faction_id: 'cosmic_scholars', services: ['info'] },
    { name: 'Rex Cargopaw', title: 'Dock Chief', race: 'muscarian', faction_id: 'traders_guild', services: ['info', 'trade'] },
    { name: 'Fen Nightshade', title: 'Informant', race: 'tarri', faction_id: 'shadow_syndicate', services: ['info'] },
    { name: 'Bolg Ironfist', title: 'Guard Captain', race: 'kalin', faction_id: 'frontier_rangers', services: ['quest'] },
  ];
