...
export async function up(knex: Knex): Promise<void> {
  // 1. Add fame and infamy columns to player_faction_rep
  await knex.schema.alterTable('player_faction_rep', (t) => {
    t.integer('fame').notNullable().defaultTo(0);
    t.integer('infamy').notNullable().defaultTo(0);
  });
  ...
  // 5. Seed rivalry rows (bidirectional)
