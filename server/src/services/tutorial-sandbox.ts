...
async function persistState(playerId: string, state: TutorialVirtualState): Promise<void> {
...
function outpostPrices(outpost: TutorialOutpost) {
  return {
    cyrillium: {
      price: calculatePrice('cyrillium', outpost.cyrillium.stock, outpost.cyrillium.capacity),
...
export async function handleTutorialSector(req: Request, res: Response) {
  try {
    ...
    if (!sector) return res.status(404).json({ error: 'Tutorial sector not found' });
    ...
    res.json({
      ...
      adjacentSectors: sector.adjacentSectors.map(id => ({ sectorId: id, oneWay: false })),
      players: [],
      outposts: sector.outpost ? [{ id: sector.outpost.id, name: sector.outpost.name }] : [],
      planets: [],
      deployables: [],
      events: [],
      warpGates: [],
...
export async function handleTutorialMove(req: Request, res: Response) {
  try {
    ...
    if (!currentSector) return res.status(400).json({ error: 'Current tutorial sector invalid' });
    ...
    if (!targetSector) return res.status(400).json({ error: 'Target sector not found' });
...
export async function handleTutorialBuy(req: Request, res: Response) {
  try {
    ...
    if (commData.mode !== 'sell') {
      return res.status(400).json({ error: 'Outpost does not sell this commodity' });
    }

    const currentCargo = state.cyrilliumCargo + state.foodCargo + state.techCargo + state.colonistsCargo;
    ...
    if (maxBuyable <= 0) {
      return res.status(400).json({ error: 'No cargo space or stock available' });
    }

    const pricePerUnit = calculatePrice(commodity as CommodityType, commData.stock, commData.capacity);
    ...
    const cargoKey = `${commodity}Cargo` as keyof Pick<TutorialVirtualState, 'cyrilliumCargo' | 'foodCargo' | 'techCargo'>;
...
export async function handleTutorialSell(req: Request, res: Response) {
  try {
    ...
    if (commData.mode !== 'buy') {
      return res.status(400).json({ error: 'Outpost does not buy this commodity' });
    }
...
export async function handleTutorialScan(req: Request, res: Response) {
  try {
    const state = parseTutorialState(req);
    const sector = getTutorialSector(state.currentSectorId);
    if (!sector) return res.status(400).json({ error: 'Current sector invalid' });
...
export async function resetPlayerForRealGame(playerId: string): Promise<{ newSectorId: number; newCredits: number }> {
  ...
  const startingCredits = GAME_CONFIG.STARTING_CREDITS + (raceConfig?.startingCreditsBonus ?? 0);
  const startingMaxEnergy = GAME_CONFIG.MAX_ENERGY + (raceConfig?.startingMaxEnergyBonus ?? 0);
